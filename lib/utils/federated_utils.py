...
# 仅聚合source domain的模型
def federated_avg(model_list, client_weights, mode='avg'):
    """
    ...
    with torch.no_grad():
        ...
        #         if 'bn' not in key:
        #             temp = torch.zeros_like(model_list[0].state_dict()[key], dtype=torch.float32)
        #             for client_idx in range(len(client_weights)-1):
        #                 temp += client_weights[client_idx+1] * model_list[client_idx+1].state_dict()[key]
        #             model_list[0].state_dict()[key].data.copy_(temp)
        #             for client_idx in range(len(client_weights)-1):
        #                 model_list[client_idx+1].state_dict()[key].data.copy_(model_list[0].state_dict()[key])
        if mode == 'fedbn':
            for key in model_list[0].state_dict().keys():
                if 'bn' not in key:
                    temp = torch.zeros_like(model_list[0].state_dict()[key], dtype=torch.float32)
                    for client_idx in range(len(client_weights)):
                        temp += client_weights[client_idx] * model_list[client_idx].state_dict()[key]
                    model_list[0].state_dict()[key].data.copy_(temp)
                    for client_idx in range(len(client_weights)):
                        model_list[client_idx].state_dict()[key].data.copy_(model_list[0].state_dict()[key])
        else:
            for key in model_list[0].state_dict().keys():
                # num_batches_tracked is a non trainable LongTensor and
                # num_batches_tracked are the same for all clients for the given datasets
                if 'num_batches_tracked' in key:
                    model_list[0].state_dict()[key].data.copy_(model_list[0].state_dict()[key])
                else:
                    temp = torch.zeros_like(model_list[0].state_dict()[key])
                    for client_idx in range(len(client_weights)):
                        temp += client_weights[client_idx] * model_list[client_idx].state_dict()[key]
                    model_list[0].state_dict()[key].data.copy_(temp)
                    for client_idx in range(len(client_weights)):
